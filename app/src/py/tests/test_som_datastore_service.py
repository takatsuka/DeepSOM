...
def test_import_data_from_csv_duplicate(mocker):
    mock_window = Mock()
    mock_window.create_file_dialog.return_value = ["tests/resources/test_csv.txt"]
    mocker.patch.object(webview, "windows", [mock_window])
...
def test_import_duplicate_json(mocker):
    mock_window = Mock()
    mock_window.create_file_dialog.return_value = ["tests/resources/test_json.json"]
    mocker.patch.object(webview, "windows", [mock_window])
...
def test_load_workspace_success(mocker):
    ...
    assert ds.ws_path == "tests/resources/test_workspace.json"
    file_spy.assert_called_once()
...
def test_save_current_workspace(mocker):
    mock_window = Mock()
    mock_window.create_file_dialog.return_value = "tests/resources/test_workspace"
    mocker.patch.object(webview, "windows", [mock_window])
...
def test_save_workspace_as(mocker):
    ...
    spy.assert_called_once()
    assert ds.ws_path == "tests/resources/test_workspace"

def test_save_workspace(mocker):
    mock_window = Mock()
    mock_window.create_file_dialog.return_value = "tests/resources/test_workspace_actual.json"
    mocker.patch.object(webview, "windows", [mock_window])
    ...
    assert ds.ws_path == "tests/resources/test_workspace_actual.json"
    assert filecmp.cmp("tests/resources/test_workspace_actual.json", "tests/resources/test_workspace_expected.json")
