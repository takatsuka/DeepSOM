...
from som_view_service import SomViewService
...
def test_init():
    ...
    assert service.links == None
    assert service.nodes == None

def test_generate_som_nodes():
    mock_som = MagicMock(spec=SOM)
    mock_som.size = 2
    service.som = mock_som
    assert service.generate_nodes() == [{'id': 0}, {'id': 1}, {'id': 2}, {'id': 3}]
...
def test_rect_links(mocker):
    ...
    mock_som.size = 2
    service.som = mock_som
    assert service.rect_links() == [{'source': 0,'target': 1,'value': 0.0},{'source': 2,'target': 3,'value': 0.0},{'source': 0,'target': 2,'value': 0.0},{'source': 1,'target': 3,'value': 0.0}]
...
def test_set_input_bad_nodes():
    ...
    service.generate_nodes = MagicMock(return_value=None, side_effect=Exception)
